...
void eval(char *cmdline) 
{
  ...
  int bg = parseline ( cmdline, argv );//indicate bg or fg
...
{
  struct job_t *job;
  //if the argument is not valid(no PID or JID)
  if (argv[1]==NULL){
    printf("%s command requires PID or %%jobid argument\n", argv[0]);
    return;
  }
  //if PID/JID not valid
  ...
  //if JID entered
  if (argv[1][0] == '%'){
    job = getjobjid(jobs, atoi(&argv[1][1]));//get the job
    if (job == NULL) {
      printf("%s: No such job\n", argv[1]);//print no such job
      return;
    }
  ...
  //if PID entered
    ...
    job = getjobjid(jobs, atoi(argv[1]));//get the job
    if (job == NULL){
      printf("(%d): No such process\n", atoi(argv[1]));//print no such process
      return;
    }
  }
  //BG->FG
...
void waitfg(pid_t pid)
{
  ...
  if (job!=NULL){
    //sleep if job is foreground process
...
void sigint_handler(int sig) 
{
  ...
  //send SIGINT to entire group of process with pid
  if (pid != 0) {
    kill (-pid, sig);
  }
  return;
...
{
  pid_t pid = fgpid(jobs);
  //send SIGTSTP to entire group of process with pid
  if (pid != 0) {
    kill (-pid, sig);
  }
  return;
